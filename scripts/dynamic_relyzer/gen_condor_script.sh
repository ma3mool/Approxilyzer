...
#SIMICS_DIR=/shared/workspace/approx_comp/full_relyzer/src_GEMS/simics/home/dynamic_relyzer/   # Simics directory to run experiments from 
SIMICS_DIR=/shared/workspace/approx_comp/scripts/dynamic_relyzer/ #directory with scripts
CONDOR_SCRIPTS_PATH=/shared/workspace/approx_comp/full_relyzer/src_GEMS/simics/home/dynamic_relyzer/condor_scripts
...
header()
{
    ...
    if [ $1 = "static" ]
    then
        executable="$SIMICS_DIR/run_static_analysis_with_cmd_line.sh"
    fi

    if [ $1 = "dynamic" ] 
    then
        executable="$SIMICS_DIR/run_dynamic_analysis_with_cmd_line.sh"
    fi
