...
# bit is now multiple bit flips
def run_new_all(app, phase, seqnum, pc, core=-1,  type=-1, bits=-1, stuck=-1, faultreg=64, srcdest=0, pilot=0, injcycle=0):
...
#    start_points = [ 330627258853,  330727258853,  330827258853,  330927258853,  331027258853,  331127258853,  331227258853,  331327258853,  331427258853 ]
  ...
  run_name = '%s.pc%s.p%s.c%s.t%s.b%s.s%s.r%s.d%s.%s' % (app, pc, pilot, core, type, bits, stuck, faultreg, srcdest, seqnum)

  if ARCH_INJ == 1:
    filename = "/dev/null"
    run_sim_command('c %d' %(INJ_INST))
    inject_trans_fault(pc, faultreg, bits, srcdest, type)
    if APP_DETECTORS == 1:
...
# REMEMBER TO MAKE THIS SCALABLE GOLDEN
  ...
  run('new-file-cdrom /shared/workspace/approx_comp/workloads/iso/coverage_output.iso')
...
#  run('cd25B_2_6.insert blackscholes_input_outcomes')
  ...
  run('cd25B_2_6.insert coverage_output')
...
def inject_trans_fault(pc, faultreg, bits, srcdest, faulttype):
  int_pc = int(pc,16)
  curr_pc = SIM_get_program_counter(conf.cpu0)
  if curr_pc == int_pc:
...
# read a fp
    ...
    if srcdest == 0:
      ...
      faulty_val = get_injected_val(golden_val,bits)
