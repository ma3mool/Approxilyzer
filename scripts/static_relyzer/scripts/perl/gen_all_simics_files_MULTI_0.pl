...
# Code added by Lin for multibit flipping
my @mutliConf = ();
push @mutliConf, "LSB2";
push @mutliConf, "LSB4";
push @mutliConf, "LSB8";
...
#push @mutliConf, "MSB4";
#push @mutliConf, "MSB8";
#push @mutliConf, "RAND2";
#push @mutliConf, "RAND4";
#push @mutliConf, "RAND8";
#push @mutliConf, "INVERTALL";
#push @mutliConf, "STRIDE4";
#push @mutliConf, "STRIDE8";
...
foreach $cov (@metric_coverage) {
	...
	#		write simics file for pilot 
			...
			for($i = $low; $i < $high; $i=$i+$BIT_GAP) {
                # Code block added by Lin to convert $i into a bitmap for multiple injection
                # We define 0's in the bitmap as the locations of flipped bits
                # fault_type 12 is 32bits and others are 64bits
                my $bitmask_c = (0b1 << $i); # Create the complement of the desired bitmap
                my $bitmask = 0; # Declear the true bitmask
                my $bmap; # Declear the string in correct output format
                if(($fault_type+0)==12) {
                    # in the case of 32bits float
                    $bitmask = (0xFFFFFFFF)^$bitmask_c;
                    # bitmask = all 1's Xor the complement of bitmask
                    # NOTE:This is necessary because built-in types in perl is 64bits. If take the complement directly,
                    # the result is 64bits
                    $bmap = sprintf("%#.8x",$bitmask);
                } else {
                    # in the case of 64bits int/double
                    $bitmask = (0xFFFFFFFFFFFFFFFF)^$bitmask_c;
                    # Use explict magic number to avoid platform specific data width
                    $bmap = sprintf("%#.16x",$bitmask);
                }
				# my $job_name = "$app_name.$pc.$adjusted_pilot.$c.$arch_reg.$i.$src_dest.$fault_type";
				my $job_name = "$app_name.$pc.$adjusted_pilot.$c.$arch_reg.$bmap.$src_dest.$fault_type";
				if($write_files == 1) {
            ...
            # TODO: Multibits injects logic goes here.
            # LSB 2,4,8    MSB 2,4,8    2,4,8 random    invert_all    every 4,8 bits
            #
            # Code block added by Lin to enable mutlibits injection
            # This code block iterates through multiConf array and apply inject for each setting
            my $mconf; # Declear the config variable
            foreach $mconf (@mutliConf) {
                my $bitmask_c; # Complement of the bitmask
                my $bitmask = 0; # Declear the true bitmask
                my $bmap; # Declear the string in correct output format
                switch($mconf) {
                    case "LSB2" {$bitmask_c = (0b11)}
                    case "LSB4" {$bitmask_c = (0b1111)}
                    case "LSB8" {$bitmask_c = (0b11111111)}
                    # TODO: Add cases for other configurations here
                }
                # After switch statement, complement of bitmask has been generated
                # Now generate the true bitmask based on data type
                if(($fault_type+0)==12) {
                    # in the case of 32bits float
                    $bitmask = (0xFFFFFFFF)^$bitmask_c; 
                    $bmap = sprintf("%#.8x",$bitmask);
                ...
                # Blow is the logic to printf into output file
