...
foreach $cov (@metric_coverage) {
	...
	#		write simics file for pilot 
			...
			for($i = $low; $i < $high; $i=$i+$BIT_GAP) {
                ...
                # We define 1's in the bitmap as the locations of flipped bits
                # fault_type 12 is 32bits and others are 64bits
                my $bitmask = (0b1 << $i); # Create the complement of the desired bitmap
            ...
            foreach $mconf (@mutliConf) {
                my $bitmask; # Complement of the bitmask
                #my $bitmask = 0; # Declear the true bitmask
                my $bmap; # Declear the string in correct output format
                switch($mconf) {
                    case "LSB2" {$bitmask = (0b11)}
                    case "LSB4" {$bitmask = (0b1111)}
                    case "LSB8" {$bitmask = (0b11111111)}
