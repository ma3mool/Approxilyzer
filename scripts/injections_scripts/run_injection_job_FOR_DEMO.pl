...
while ($num_proceeded >= 1) {
    ...
    print "Injection run...\n";
	$result = `$SIMICS_PATH/simics -stall $simics_file`;
	...
	if (-e "$checkpoint_filename")
	{
		...
		$no_print = 0;
        # MAKE SCALABLE
        #my $sd1_config = "/home/pramach2/research/GEMS/simics/home/spec_opensolaris/sd1_config" ;
        my $sd1_config = "/home/venktgr2/checkpts/spec_opensolaris/sd1_config" ;
		my $add_sd1 = 1 ;
		my $has_sd1 = 1 ;
		while (<IN_CONFIG>) {
		    $line = $_;
