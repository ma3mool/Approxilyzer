...
DYNAMIC_RELYZER_SCRIPTS=$RELYZER_SHARED/scripts/dynamic_relyzer
...
############################## PHASE 3 of RELYZER ############################## 
    ...
    cd $RELYZER_SHARED/scripts/static_relyzer/src
    make clean; make

    cd $RELYZER_SHARED/scripts/static_relyzer
 #   app_name="$(python $RELYZER_SHARED/scripts/dynamic_relyzer/get_app_name.py $1)"
    app_name=$1
    ...
    $RELYZER_SHARED/scripts/static_relyzer/src/relyzer-front-end $RELYZER_APPS/program_list.txt

    # generate fault list
    $RELYZER_SHARED/scripts/static_relyzer/scripts/bash/gen_all_simics_files.sh $1
...
if [ $2 -eq "4" ]; then
    ...
    cd $GEMS_PATH_SHARED/simics/home/dynamic_relyzer/
    ./simics -stall $RELYZER_SHARED/workloads/apps/${1}/simics_files/${1}_create_checkpoints.simics
fi
