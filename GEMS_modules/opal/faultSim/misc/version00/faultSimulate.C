...
faultSimulator::faultSimulator(){
	numGateEval = 0;
	numEvents = 0;
	// FIX static vars are already initialized to 0!
	numInvoke = 0;
	numCorrupt = 0;
	counter = 0;
}
...
//	Purpose: Filling in stimuli table line by line by reading-in the .vec file
//	Parameters: string line: line of file containing gate data
//	Returns: -
// ******************************************************************************
/*
void faultSimulator::extractStimuli(string line){
	...
	for(i = 0; i < design.numPI; i++){
		//handling X, X is defined as a proper int value in includes.h
		if( (line[i] == 'X')||(line[i] == 'x') ){
			readIn.push_back(logic_X);
		}
		else if( (line[i] == 'Z')||(line[i] == 'z') ){
			readIn.push_back(logic_Z);
		}
		else{		
			// converting char to int
			readIn.push_back(line[i]-'0');
		}
	}

	//FIX!! - map readIn > A B cin
	permuted[0] = readIn[0];
	permuted[1] = readIn[32];
	permuted[2] = readIn[64];
	i = 1;
	int index;
	while(i < 32){
		index = 2*i + 1;
		permuted[index] = readIn[i];
		permuted[index + 1] = readIn[i+32];
		i++;
	}
...
//	Purpose: Reading-in <circuit>.vec file 
//	Parameters: string vecFileName: Name of the file containing input stimuli 
//	Returns: -
// ***************************************************************************
/*
void faultSimulator::vecRead(string vecFileName)
{
	int lineCount = 0;
	// each line of the vec file corresponds to an input stimuli
	string line;
	ifstream vec;
	vec.open(vecFileName.c_str());

	if(!vec) throw "Cannot open .vec file...";
...
// wrapper for extractStimuli: TODO optimize
...
//	}// end for: traversing the input stimuli list
}

type faultSimulator::writeResults(){

//	valueString response[REG_WIDTH];

	char response[REG_WIDTH];
...
void faultSimulator::injectRandomFault()
{
	...
	// TODO a PO does not neccessarily have one output!! There is a stem, one branch gets read as PO, there may be several other branches
	// feeding the circuit!
	//
	// however, this is not the case for the adder at hand
	//
...
//	printf("Number of outputs of the faulty gate: %d\n", design.circuit[faultyGateId].gateDescription[NUMO]);
	...
	if(!faultyValue){
		...
		// TEST
//		cout << "Inserting OR gate " << g.gateDescription[ID] << endl;
	}
...
//	g.gateDescription[C1] = design.circuit[faultyGateId].gateDescription[C1];
	...
	if(design.circuit[faultyGateId].gateDescription[NUMO]==0){		
		...
		if(design.saf.faultyFanoutIndex == 0){
			...
			// TEST
//			cout << "faulty fanout gate " << faultyFanOutId << endl;
...
/*
void faultSimulator::writeResults(){

	// each line of .out file corresponds to circuit response
	// under an input stimuli
	ofstream response;
	
	string suffix = "Sim.out";	
	string outFileName = design.moduleName + suffix;
	
	// open the output file 
	response.open(outFileName.c_str(),ios::app);
	
	int value;

	if(!response) throw "faultSimulator::writeResults> Cannot open .out file...";

	int it;
	if(response.is_open()){
		// Outputs are specified in the order read from .lev file
		for(it = 0; it < design.POList.size()-2; it++){
			value = decode( design.circuit[design.POList[it]].value );
			if (value == logic_X)
				response << "X";
			else if (value == logic_Z)
				response << "Z";
		...
		value = decode( design.circuit[design.POList[it+1]].value );
		if (value == logic_X)
			response << "X";
		else if (value == logic_Z)
			response << "Z";
